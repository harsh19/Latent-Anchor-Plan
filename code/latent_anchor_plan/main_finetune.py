...
parser.add_argument('--init_eval', type=str2bool, default='false')
parser.add_argument('--wait_for_reward_to_sabilize', type=str2bool, default='false')
...
def get_posterior_loss_weight(epoch, batch):
    if args.wait_for_reward_to_sabilize and epoch==1 and batch<20:
        return 0.0
    return args.posterior_loss_wt
...
def train():
    ...
    while batch < num_batches: # - 1 - 1:
        ...
        if args.prior_regularize:
            ...
            #new
            prior_reg_rewards[samples_reg == pad_id] -= 1.0


        #####---------loss computation and backprop
        kl_weight = kl_anneal_function(step=num_train_steps)
        # new
        if kl_loss.mean()<(kl_loss.mean()-kl_loss.mean()+30):
            kl_loss*=0.0
        ...
        posterior_loss_wt = get_posterior_loss_weight(epoch, batch)
        loss += posterior_loss_wt*posterior_loss
        ...
        if batch % args.log_interval == 0: # and batch > 0:
            ...
            if args.finetune:
                args.outf = 'tmp/outputs/' + args.run_name + '.epoch' + str(epoch) + str(batch) + '.batch.storysamples'
                args.sents = 101
                args.words = 100
                generate_story_samples(model, val_data, args, corpus)
                d = diversity_evals.read_file(args.outf, 'plot', one_per_title=True)
                diversity_evals_story = diversity_evals.diversity_entropy(d)
                print("epoch = ", epoch, "batch = ", batch, "diversity_evals: generate_story_samples: stories: ", diversity_evals_story)
                diversity_evals_plots = diversity_evals.diversity_entropy(d, use_plots=True)
                print("epoch = ", epoch,"batch = ", batch,  "diversity_evals: generate_story_samples: plots: ", diversity_evals_plots)
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if (args.train_inference_nw_only or args.init_eval) and epoch == 1:
            ...
            if args.init_eval:
                ...
                args.outf = 'tmp/outputs/' + args.run_name + '.epoch.init' + '.storysamples'
                args.sents = 151
                args.words = 100
                generate_story_samples(model, test_data, args, corpus, use_provided_plots=args.pretrain_ae_mode)
                d = diversity_evals.read_file(args.outf, 'plot', one_per_title=True)
                diversity_evals_d = diversity_evals.diversity_entropy(d)
                print("*BEGINNING* diversity_evals: generate_story_samples: stories: ", diversity_evals_d)
                diversity_evals_plots = diversity_evals.diversity_entropy(d, use_plots=True)
                print("*BEGINNING* diversity_evals: generate_story_samples: plots: ", diversity_evals_plots)
        ...
        if args.finetune:
            ...
            d = diversity_evals.read_file(args.outf, 'plot', one_per_title=True)
            diversity_evals_story = diversity_evals.diversity_entropy(d)
            print("epoch = ", epoch, "diversity_evals: generate_story_samples: stories: ", diversity_evals_story)
            diversity_evals_plots = diversity_evals.diversity_entropy(d, use_plots=True)
            print("epoch = ", epoch, "diversity_evals: generate_story_samples: plots: ", diversity_evals_plots)
        else:
